...
class VideoFramesDataset(Dataset):
    def __init__(self, video_list: List[Tuple[str, str, int]],
        ...
        frame_cnt_list = [0, *[math.ceil(v[-1]) for v in video_list]]
        self.offset_list = np.cumsum(frame_cnt_list).tolist()

    def __getitem__(self, item):
        video_idx, frame_idx = self.offset_to_index(item)
        vid, _ = self.video_list[video_idx]
        if vid[0] == "R":
            vdir = self.root + "reference/" + vid + "/" + vid + "_"
        else:
            vdir = self.root + "query/" + vid + "/" + vid + "_"
        path = vdir + str(frame_idx).zfill(4) + ".png"
        # path = self.id_to_key_fn(vdir, frame_idx)
        path = os.path.join(self.root, path) if self.root else path

        # Get value from the Train data set
        try:
            value = self.reader.read(path)
        # Get value from the Val data set
        except:
            if vid[0] == "R":
                vdir = self.root[:-6] + "val/reference/" + vid + "/" + vid + "_"
            else:
                vdir = self.root[:-6] + "val/query/" + vid + "/" + vid + "_"
            path = vdir + str(frame_idx).zfill(4) + ".png"
            # path = self.id_to_key_fn(vdir, frame_idx)
            path = os.path.join(self.root, path) if self.root else path
            value = self.reader.read(path)
    ...
    def offset_to_index(self, offset: int):
